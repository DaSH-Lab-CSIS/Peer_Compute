...
def get_payload(benchmark_no, size):
    ...
    if size not in ['test', 'small', 'large']:
        print("Size must be one of: test, small, large")
        return {'error':'size is not one of test,small,large'}
    
    print(f"Benchmark number: {benchmark_no}")
    print(f"Size: {size}")

    if benchmark_no == '010':
        from invocations.b010 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '020':
        ...
        from invocations.b120 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '210':
        from invocations.b210 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '220':
        print("inactive")
        ...
        from invocations.b311 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '411':
        print("inactive")
    elif benchmark_no == '501':
        from invocations.b501 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '502':
        from invocations.b502 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '503':
        from invocations.b503 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    elif benchmark_no == '504':
        from invocations.b504 import generate_input_for_generator
        payload = generate_input_for_generator(size)
        pass
    else:
        print(f"Invalid benchmark number: {benchmark_no}")
