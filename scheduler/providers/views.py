...
def update_efficiency_score_in_models(
    user_id,
    provider_cpu_usage,
    provider_memory_usage,
    reference_cpu_usage,
    reference_memory_usage,
):
    print(
        "updating models... with r_cpu= ",
        reference_cpu_usage,
        " r_mem= ",
        reference_memory_usage,
        " p_cpu= ",
...
def get_benchmarks_for(user_id, benchmark):
    ...
    update_efficiency_score_in_models(
        user_id,
        provider_cpu_usage,
        provider_memory_usage,
        reference_cpu_usage,
...
def on_message(mqtt_client, userdata, msg):
    ...
    try:
        ...
        if msg.payload.decode("utf-8").startswith("Benchmark:"):
            ...
            get_benchmarks_for(
                user_id=user_id, benchmark=benchmark
            )  # this will also update models.
        ...
        if msg.topic == "EVERYONE":
            ...
            if msg.payload.decode("utf-8").startswith("get_efficiency_score"):
                ...
                provider = User.objects.get(user_id=user_id)
                scoreset = {
                    "cpu": float(provider.cpu_efficiency_score),
                    "memory": float(provider.memory_efficiency_score),
                }
                mqtt_client.publish(
                    topic=user_id,
                    payload="EfficiencyScoreSet:" + json.dumps(scoreset),
                    qos=2,
                )
...
def publish_to_topic_mqtt(
    runMultipleInvocations,
    numberOfInvocations,
    isChained,
    inputData,
    provider,
    ...
    mclient.publish(
        topic=router_name, payload=json.dumps(userdata).encode("utf-8"), qos=2
    )
...
def set_reference_stats(request):
    ...
    client.publish(
        topic=reference_provider_id, payload="ref_run_service_id/" + service_id
    )
    return JsonResponse(
        {
            "State": "Running service on the reference provider, stats will be printed on django server and added to files also"
        }
    )
...
def finish_job(data):
    ...
    job.save()
    providing_time = int(
        ((job.ack_time - job.start_time) / timedelta(microseconds=1)) / 1000
    )  # Providing time in milliseconds
    if USE_FABRIC:
        r = fabric.invoke_received_result(str(job.id))
        if (
            "jwt expired" in r.text
