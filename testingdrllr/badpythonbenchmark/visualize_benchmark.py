...
if __name__ == '__main__':
    cpu_usage, memory_usage = parse_benchmark_results('benchmark_results.txt')
    time_data = parse_time_output('benchmark_output.txt')
    
    # Plot CPU usage
    plt.figure()
    plt.bar(['CPU Usage'], [cpu_usage])
    plt.ylabel('CPU usage (nanoseconds)')
    plt.title('CPU Usage from cgroup')
    ...
    plt.title('Memory Usage from cgroup')
    ...
    plt.title('Maximum Resident Set Size')
    ...
    plt.title('User and System Time')
    plt.savefig('user_system_time.png')
    
    print("Plots saved: cpu_usage.png, memory_usage.png, max_rss.png, user_system_time.png")
