...
if __name__ == '__main__':
    cpu_usage, memory_usage = parse_benchmark_results('drl_training_benchmark_results.txt')
    time_data = parse_time_output('drl_training_benchmark_output.txt')

    # Plot CPU usage
    plt.figure()
    plt.bar(['CPU Usage'], [cpu_usage])
    plt.ylabel('CPU usage (nanoseconds)')
    plt.title('CPU Usage from cgroup (DRL Training)')
    plt.savefig('drl_training_cpu_usage.png')

    # Plot Memory usage
    plt.figure()
    plt.bar(['Memory Usage'], [memory_usage])
    plt.ylabel('Memory usage (bytes)')
    plt.title('Memory Usage from cgroup (DRL Training)')
    plt.savefig('drl_training_memory_usage.png')

    # Plot Max RSS from /usr/bin/time
    plt.figure()
    plt.bar(['Max RSS'], [time_data['max_rss']])
    plt.ylabel('Memory usage (kbytes)')
    plt.title('Maximum Resident Set Size (DRL Training)')
    plt.savefig('drl_training_max_rss.png')

    # Plot User and System time
    plt.figure()
    plt.bar(['User Time', 'System Time'], [time_data['user_time'], time_data['system_time']])
    plt.ylabel('Time (seconds)')
    plt.title('User and System Time (DRL Training)')
    plt.savefig('drl_training_user_system_time.png')

    print("Training benchmark plots saved.")
