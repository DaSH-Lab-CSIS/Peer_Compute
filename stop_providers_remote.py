#!/usr/bin/env python3
"""
Stop providers running on remote nodes.
...
def stop_provider_on_host(
    ...
    """Stop provider on a remote host."""
    # Kill any running provider1.py processes
    kill_by_process_cmd_str = "pkill -f 'python.*provider1.py' || true"
    ...
    print(f"[{host}] Stopping provider...")
    ...
    try:
        ...
        if force:
            ...
            force_kill_cmd_str = "pkill -9 -f 'python.*provider1.py' || true"
        ...
        print(f"[{host}] Provider stopped")
        return True
    except Exception as e:
        print(f"[{host}] Error stopping provider: {e}")
        return False


def main():
    parser = argparse.ArgumentParser(
        description='Stop providers on multiple remote nodes'
    )
    ...
    parser.add_argument(
        '--force',
        action='store_true',
        help='Force kill providers (SIGKILL)'
    )
    ...
    print(f"Stopping providers on {len(hosts_to_use)} host(s): {', '.join(hosts_to_use)}")
    print()
    
    # Stop providers
    for host in hosts_to_use:
        stop_provider_on_host(host, force=args.force,
                              jumpnode_password=jumpnode_password,
                              node_password=node_password,
                              ssh_config_path=ssh_config_path)
